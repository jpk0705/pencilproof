...
assert.match(deploymentConfigSource, /"crons": \["0 17 \* \* MON,WED,FRI", "10 17 \* \* MON,WED,FRI"\]/);
...
assert.doesNotMatch(analyzeSource, /REVISED QUOTE CHECK/);
assert.doesNotMatch(analyzeSource, /Compare a revised quote/);
...
assert.match(analyzeSource, /DEALER QUOTED PAYMENT/);
assert.match(analyzeSource, /COUNTER-PROPOSAL \/ LIVE/);
...
assert.match(analyzeSource, /PAID_AUDIT_URL = "https:\/\/audit\.pencilproof\.com\/analyze\/secure\/"/);
assert.match(analyzeSource, /data\.expiresAt/);
...
assert.match(wranglerSource, /"0 17 \* \* MON,WED,FRI"/);
assert.match(wranglerSource, /"10 17 \* \* MON,WED,FRI"/);
...
assert.doesNotMatch(phoneWorkerSource, /const accountUrl = \`\$\{env\.SITE_ORIGIN\}\/account\//);
