...
type AnalyticsAccountPaidFilter = "all" | "paid" | "unpaid";

const analyticsAccountPaidFilter = (value: string | null): AnalyticsAccountPaidFilter =>
...
const analyticsDashboard = async (request: Request, env: Env) => {
  ...
  const selectedAccountPaid = analyticsAccountPaidFilter(requestUrl.searchParams.get("account_paid"));
  ...
  const accounts = allAccounts.filter((account) =>
    (selectedAccountRole === "all" || account.role === selectedAccountRole)
    && (selectedAccountPaid === "all" || (selectedAccountPaid === "paid" ? account.paid : !account.paid)),
  );
