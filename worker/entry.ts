...
type AnalyticsAccountRoleFilter = "all" | "consumer" | "salesperson" | "both";

const analyticsAccountRoleFilter = (value: string | null): AnalyticsAccountRoleFilter =>
  value === "consumer" || value === "salesperson" || value === "both" ? value : "all";
...
const analyticsDashboard = async (request: Request, env: Env) => {
  ...
  const selectedRange = analyticsRange(requestUrl.searchParams.get("range"));
  const selectedAccountRole = analyticsAccountRoleFilter(requestUrl.searchParams.get("account_role"));
  const summary = await analyticsSummary(request, env);
  if (!summary) return new Response("Analytics unavailable", { status: 502 });
  const allAccounts = await analyticsAccounts(env);
  const accounts = selectedAccountRole === "all"
    ? allAccounts
    : allAccounts.filter((account) => account.role === selectedAccountRole);
  ...
  const roleQuery = selectedAccountRole === "all" ? "" : `&account_role=${encodeURIComponent(selectedAccountRole)}`;
  const rangeLinks = ANALYTICS_RANGES.map((range) => `<a class="range ${range.key === rangeKey ? "selected" : ""}" href="/analytics?range=${range.key}${roleQuery}">${esc(range.label)}</a>`).join("");
...
<section class="panel" style="margin-top:18px"><h2>Account sign-ins</h2><p class="subtle">Email addresses from successful account setup, with the sign-in context that has been used. If the same account has entered through both paths, it is labeled <strong>Both</strong>.</p><form class="account-filter" method="get" action="/analytics"><input type="hidden" name="range" value="${esc(rangeKey)}"><label for="account-role">Filter by account type<select id="account-role" name="account_role" onchange="try{sessionStorage.setItem('pencilproof-analytics-scroll-y', String(window.scrollY))}catch{} this.form.submit()"><option value="all"${selectedAccountRole === "all" ? " selected" : ""}>All accounts</option><option value="consumer"${selectedAccountRole === "consumer" ? " selected" : ""}>Consumer</option><option value="salesperson"${selectedAccountRole === "salesperson" ? " selected" : ""}>Salesperson</option><option value="both"${selectedAccountRole === "both" ? " selected" : ""}>Both paths</option></select></label><noscript><button class="download" type="submit">Apply filter</button></noscript></form><div class="feedback-grid"><div class="feedback-metric"><div class="label">Shown accounts</div><div class="big">${accounts.length.toLocaleString("en-US")}</div></div><div class="feedback-metric"><div class="label">Consumers</div><div class="big">${accountCounts.consumer.toLocaleString("en-US")}</div></div><div class="feedback-metric"><div class="label">Salespeople</div><div class="big">${accountCounts.salesperson.toLocaleString("en-US")}</div></div><div class="feedback-metric"><div class="label">Used both paths</div><div class="big">${accountCounts.both.toLocaleString("en-US")}</div></div></div>${accountRows ? `<div class="table-wrap"><table><thead><tr><th>Email</th><th>Account type</th><th>First seen</th><th>Last seen</th></tr></thead><tbody>${accountRows}</tbody></table></div>` : `<div class="empty">No accounts match this filter.</div>`}</section>
