...
export default function SalespersonPage() {
  ...
  if (!clerk.user) return <>
    ...
    <main className="sales-page shell">
      ...
      <p className="sales-promo-banner"><strong>ALPHA1 launch offer:</strong> first month $1, then $20/month unless canceled. Limited to the first 100 redemptions; availability is confirmed in checkout.</p>
  ...
  return <>
    ...
    <main className="sales-page shell">
      ...
      {!profile || !isActive(profile.subscriptionStatus) ? <p className="sales-promo-banner"><strong>ALPHA1 launch offer:</strong> first month $1, then $20/month unless canceled. Limited to the first 100 redemptions; availability is confirmed in checkout.</p> : null}
      ...
      {profile && !isActive(profile.subscriptionStatus) ? <section id="salesperson-plan" className="sales-card sales-dashboard-onboarding"><p className="kicker">SALESPERSON DASHBOARD</p><h2>{profile.subscriptionStatus === "not_started" ? `Welcome, ${profile.displayName}` : "Resume your salesperson plan"}</h2><p>Your free salesperson account is ready. Activate the plan to unlock your tracked link, QR code, referral credits, and sales tools.</p><div className="sales-plan-offer"><strong>ALPHA1 launch offer</strong><span>First month $1, then $20/month unless canceled · first 100 redemptions · checkout confirms availability</span></div><button className="button button-primary" type="button" onClick={() => void startSubscription()} disabled={busy}>{busy ? "Opening checkout…" : "Start with the $1 first month"}</button>{message ? <p className="sales-message" role="status">{message}</p> : null}</section> : null}
