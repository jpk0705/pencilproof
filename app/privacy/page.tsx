  ...
  return (
    <main className="legal-page">
      ...
      <article className="legal-copy shell">
        ...
        <h2>The short version</h2><p>The quote-audit tool and document importer run in your web browser. The financial figures you enter or import are used on your device to calculate the displayed results and are not intentionally sent to, or stored by, PencilProof.</p>
        <h2>Information processed</h2><p>The audit tool processes the vehicle description, prices, fees, products, trade information, loan rates, term, and payment you choose to enter. If you choose a digital or scanned PDF, JPG, JPEG, or PNG, compatible text is read locally in your browser to suggest values for those fields. Browser OCR software may download the code and English-language data it needs, but your selected document and extracted text are not intentionally uploaded to PencilProof. Avoid files containing Social Security numbers, bank details, driver&apos;s license numbers, or other unnecessary identifiers.</p>
        ...
        <h2>Your choices</h2><p>You can clear the audit tool at any time with the “Clear all” button or close the browser tab. Because deal inputs are not stored by PencilProof, we cannot retrieve a past audit. For checkout-data requests, use the privacy and support options provided by Link or Stripe.</p>
        <h2>Changes</h2><p>If accounts, analytics, server-based document processing, or additional email features are added, this notice will be updated before those features are made available. The effective date above will change.</p>
