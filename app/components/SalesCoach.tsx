...
const respond = (prompt: string, source: Objection[], unlocked: boolean) => {
  ...
  if (!objection) {
    return unlocked
      ...
      : { category: "More practice available", text: "That question is outside the five free preview topics. Subscribe for more objection categories and the complete coaching playbook.", allResponses: [] };
  }
...
export default function SalesCoach({ unlocked = false, playbook = null, onSubscribe, startOpen = false }: SalesCoachProps) {
  ...
  const [messages, setMessages] = useState<CoachMessage[]>([
    { role: "coach", text: "Type a customer scenario to practice. I’ll help you find a clear response when it fits the available coaching topics." },
  ]);
  ...
  return <div className={"sales-coach " + (open ? "is-open" : "")}>
    {open ? <section id="sales-coach-panel" className="sales-coach-panel" aria-label="PencilProof sales coach">
      ...
      <p className="sales-coach-note">{unlocked ? "Think of this as your answer box for dealership objections. Type what the customer says and PencilProof finds a relevant coaching response in seconds. Ask first, then reveal other approaches when you want them." : "Free preview: five common dealership objections. You can choose one or type a customer scenario, then subscribe to unlock the complete categorized playbook."}</p>
